#!/usr/bin/env bash
#
# zmeshopt — README.md's Usage snippet, extracted from examples/usage.zig.
#
...
imports = [
    line for line in text.splitlines() if line.startswith('const zmeshopt = @import(')
]
if len(imports) != 1:
    sys.exit("%s: expected exactly one `const zmeshopt = @import(...)` line" % source)
